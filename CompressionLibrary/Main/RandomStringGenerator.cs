...
namespace CompressionLibrary.Main
{
    public static unsafe class RandomStringGenerator
    {
        ...
        public static void FillRandomAtoP(Span<char> output)
        {
            InitState();
            ...
            fixed (char* pOutput = output)
            {
                if (Avx2.IsSupported)
                {
                    ...
                    while (i + BlockCount <= length)
                    {
                        ...
                        Vector256<byte> nibbles = Avx2.And(vec, mask0F);
                        Vector256<byte> ascii = Avx2.Add(nibbles, baseA);

                        // Convert lower 16 bytes to 16 shorts (chars)
                ...
                // Scalar tail
                for (; i < length; i++)
                    output[i] = (char)('A' + ((int)NextU64() & 0xF));
            }
        }
    }
}
